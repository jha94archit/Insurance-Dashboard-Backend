...
class PolicySerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        fuel_instance = Fuel.objects.get(type=fuel_data.get('type'))
        instance.fuel = fuel_instance

        vehicle_segment_data = validated_data.pop('vehicle_segment')
        vehicle_segment_instance = Vehicle_Segment.objects.get(
            segment=vehicle_segment_data.get('segment'))
        instance.vehicle_segment = vehicle_segment_instance

        customer_income_group_data = validated_data.pop(
            'customer_income_group')
        customer_income_group_instance = Customer_Income_Group.objects.get(
            income_group=customer_income_group_data.get('income_group'))
        instance.customer_income_group = customer_income_group_instance

        customer_region_data = validated_data.pop('customer_region')
        customer_region_instance = Customer_Region.objects.get(
            region=customer_region_data.get('region'))
        instance.customer_region = customer_region_instance
